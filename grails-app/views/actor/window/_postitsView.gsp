...
<is:backlogElementLayout
        ...
        dblclickable='[restrictOnNotAccess:"productOwner()",
                               selector:".postit",
                               callback:is.quickLook(params:"\"actor.id=\"+obj.attr(\"elemId\")")]'
        var="actor">
  ...
  <is:postit id="${actor.id}"
          ...
          controller="actor">
    <is:truncated size="60">${actor.description?.encodeAsHTML()}</is:truncated>

  %{--Embedded menu--}%
    <is:postitMenu id="${actor.id}" contentView="window/postitMenu" params="[id:id, actor:actor]" renderedOnAccess="productOwner()"/>

    <g:if test="${actor.name?.length() > 17 || actor.description?.length() > 70}">
    <is:tooltipPostit
            id="${actor.id}"
