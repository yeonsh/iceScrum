...
<is:backlogElementLayout
  ...
  <is:postit
          ...
          controller="feature">
  <is:truncated size="60">${feature.description?.encodeAsHTML()}</is:truncated>

    %{--Embedded menu--}%
    <is:postitMenu id="${feature.id}" contentView="window/postitMenu" params="[id:id, feature:feature]" renderedOnAccess="productOwner()"/>

    <g:if test="${feature.name?.length() > 17 || feature.description?.length() > 70}">
    <is:tooltipPostit
            type="feature"
            id="${feature.id}"
