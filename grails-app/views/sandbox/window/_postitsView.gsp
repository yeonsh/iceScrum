...
<is:backlogElementLayout
        ...
        dblclickable='[restrictOnNotAccess:"productOwner()",selector:".postit",callback:is.quickLook(params:"\"story.id=\"+obj.attr(\"elemId\")")]'
        value="${stories}"
        var="story">
  ...
  %{--Embedded menu--}%
    ...
    <g:if test="${story.name?.length() > 17 || is.storyTemplate(story:story).length() > 60}">
      <is:tooltipPostit
              type="story"
              id="${story.id}"
