...
<is:wizard next="is.dialog.wizard.next" cancel="is.button.cancel" previous="is.dialog.wizard.previous" submit="is.dialog.wizard.submit" id="project-wizard" controller="project" before="\$('#choose-select-is-team-teams').empty();\$('#choose-select-is-team-members').empty();" onSuccess="\$('#dialog').dialog('close');" update="dialog" action="save">
  ...
  <is:fieldset title="is.dialog.wizard.section.project" description="is.dialog.wizard.section.project.description">
    ...
    <is:fieldDatePicker for="productstartDate" label="is.dialog.wizard.project.startDate">
      ...
      var endDateProject = new Date(new Date(dateText).getTime() + 3*24*60*60*1000);
      var endDate = new Date(new Date(dateText).getTime() + 90*24*60*60*1000);
      var endDateFirstSprint = new Date(new Date(dateText).getTime() + 88*24*60*60*1000);

      \$('#datepicker-productendDate').datepicker('option', {minDate:endDateProject,defaultDate:endDate,maxDate:null});
      \$('#datepicker-productendDate').datepicker('setDate',endDate);

      \$('#datepicker-firstSprint').datepicker('option', {minDate:startDate,defaultDate:startDate,maxDate:endDateFirstSprint});
      \$('#datepicker-firstSprint').datepicker('setDate',startDate);
